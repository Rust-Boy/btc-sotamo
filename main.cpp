...
// CCriticalSection 是一个临界区（Critical Section）的封装类，用于实现线程同步和互斥。它是比特币代码中常用的一种同步方式，可以保证多个线程对共享资源的访问顺序和正确性。
...
// 用于存储孤块（Orphan Block）。孤块是指没有被包含在当前区块链上的区块
...
// 是将一个密钥添加到钱包（Wallet）中。
// 该函数会在钱包数据库中生成相应的地址和公钥，并将私钥与地址关联起来，以便后续可以使用该私钥进行交易签名
bool AddKey(const CKey& key)
{
    CRITICAL_BLOCK(cs_mapKeys)
    {
        mapKeys[key.GetPubKey()] = key.GetPrivKey();
        // 将一个公钥（public key）与一个"CPubKey"对象实例关联起来。
        // 这个映射通常用于管理钱包中存储的公钥信息，以便在需要时可以快速查找和访问。
        mapPubKeys[Hash160(key.GetPubKey())] = key.GetPubKey();
    }
    return CWalletDB().WriteKey(key.GetPubKey(), key.GetPrivKey());
}

// 生成一个新的密钥对（public key和private key），并将其添加到钱包中
vector<unsigned char> GenerateNewKey()
{
    //CKey ，代表了比特币交易中使用的私钥（private key）。
    // 它的作用是提供一种方便的方式来操作和管理私钥信息，并且可以用于进行签署交易
    CKey key;
    key.MakeNewKey();
    if (!AddKey(key))
